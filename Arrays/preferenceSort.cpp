...
3) 'a' can be swapped only 1 time, 'b' can be swapped max 2 times,'c' can be swapped 3 times ...... z can be swapped
max 26 times.
...
void main () {
	...
	for(i=0 ; i<8 ; i++) {
		minindex = i;
		// find the max preference character / min rank
		for (j=i ; j<8 ; j++)
			if(rank[arr[j]-'a'] < rank[arr[minindex]-'a'])
				minindex = j;
		// swap it with i th character and update swap count for those indices
		if (i != minindex) {
			swaps[arr[i]-'a']++;
